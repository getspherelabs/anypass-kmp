  ...
  override fun reduce(
    ...
    return when (currentWish) {
      ...
      is NewTokenWish.OnServiceNameChanged -> {
        expect {
          currentState.copy(
              serviceNameFailed = false,
              serviceName = currentWish.serviceName,
          )
        }
      }
      ...
      is NewTokenWish.OnTypeExpandChanged -> {
        expect { currentState.copy(isTypeExpanded = currentWish.isExpanded) }
      }
