...
sealed class CipherPadding {
    ...
    protected open fun paddingSize(dataSize: Int, blockSize: Int): Int =
        blockSize - dataSize % blockSize

    protected open fun addInternal(result: ByteArray, dataSize: Int, paddingSize: Int): Unit = Unit
    protected open fun removeInternal(data: ByteArray): Int =
        data.size - (data[data.size - 1].toInt() and 0xFF)
}
...
private object CipherPaddingISO10126 : CipherPadding() {
    override fun addInternal(result: ByteArray, dataSize: Int, paddingSize: Int) {
        val randomBytes = buildSecureRandom().nextBytes(paddingSize)
