...
class Sha512 : Digest {
    ...
    override fun digest(
        ...
        if (unprocessedLimit > 0) {
            if (unprocessedLimit + length < 128) {
    ...
    private fun compress(input: ByteArray, pos: Int) {
        ...
        var localPos = pos
        for (w in 0 until 16) {
            words[w] = ((input[localPos++].toLong() and 0xff) shl 56) or
                ((input[localPos++].toLong() and 0xff) shl 48) or
                ((input[localPos++].toLong() and 0xff) shl 40) or
                ((input[localPos++].toLong() and 0xff) shl 32) or
                ((input[localPos++].toLong() and 0xff) shl 24) or
                ((input[localPos++].toLong() and 0xff) shl 16) or
                ((input[localPos++].toLong() and 0xff) shl 8) or
                ((input[localPos++].toLong() and 0xff))
        }
