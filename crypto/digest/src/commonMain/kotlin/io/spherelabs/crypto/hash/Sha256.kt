...
* https://github.com/square/okio/blob/master/okio/src/hashFunctions/kotlin/okio/internal/Sha256.kt
*/
...
class Sha256 : Digest {
    ...
    override fun digest(
        ...
        if (unprocessedLimit > 0) {
            if (unprocessedLimit + length < 64) {
                ...
                buffer.copyInto(unprocessed, unprocessedLimit, pos, limit)
            ...
            buffer.copyInto(unprocessed, unprocessedLimit, pos, pos + consumeByteCount)
            compress(unprocessed, 0)
        ...
        while (pos < limit) {
            ...
            if (nextPos > limit) {
                // Not enough bytes for a chunk.
                buffer.copyInto(unprocessed, 0, pos, limit)
    ...
    private fun compress(input: ByteArray, pos: Int) {
        val words = this.words

        var localPos = pos
        for (w in 0 until 16) {
            words[w] = (((input[localPos++] and 0xff) shl 24) or
                ((input[localPos++] and 0xff) shl 16) or
                ((input[localPos++] and 0xff) shl 8) or
                ((input[localPos++] and 0xff)))
        }

        for (w in 16 until 64) {
            val w15 = words[w - 15]
            val s0 =
                ((w15 ushr 7) or (w15 shl 25)) xor ((w15 ushr 18) or (w15 shl 14)) xor (w15 ushr 3)
            val w2 = words[w - 2]
            val s1 =
                ((w2 ushr 17) or (w2 shl 15)) xor ((w2 ushr 19) or (w2 shl 13)) xor (w2 ushr 10)
            val w16 = words[w - 16]
            val w7 = words[w - 7]
