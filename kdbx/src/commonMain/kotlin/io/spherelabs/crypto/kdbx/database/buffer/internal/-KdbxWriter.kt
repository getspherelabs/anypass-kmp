package io.spherelabs.crypto.kdbx.database.buffer.internal

import io.spherelabs.crypto.kdbx.database.header.OuterHeaderOption
import io.spherelabs.crypto.kdbx.database.header.*
...
internal fun BufferedSink.commonWriteInnerHeader(header: KdbxInnerHeader) {
    ...
    // Write stream cipher type
    writeByte(KdbxInnerHeader.CIPHER)
    writeIntLe(Int.SIZE_BYTES)
    writeIntLe(header.streamCipher.ordinal)

    // Write stream key
    writeByte(KdbxInnerHeader.KEY)
    writeIntLe(header.streamKey.size)
    write(header.streamKey)

    // Write binary data
    for ((_, item) in header.binaries) {
        val data = item.getContent()
        writeByte(KdbxInnerHeader.BINARY)
        writeIntLe(data.size + 1)
        writeByte(if (item.memoryProtection) 0x1 else 0x0)
        write(data)
    }

    // Write end marker
    writeByte(KdbxInnerHeader.END_MARKER)
    writeIntLe(0)
}
