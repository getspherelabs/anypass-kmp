...
import io.spherelabs.crypto.kdbx.database.xml.XmlTags
...
data class Meta(
    ...
    val maintenanceHistoryDays: Int = Defaults.MAINTENANCE_HISTORY_DAYS,
    ...
    val historyMaxItems: Int = Defaults.HISTORY_MAX_ITEMS,
    val historyMaxSize: Int = Defaults.HISTORY_MAX_SIZE,
...
internal object Defaults {
    const val GENERATOR = "Kdbx"
    const val PLACEHOLDERS_MAX_DEPTH = 10U
    const val MAINTENANCE_HISTORY_DAYS = 365
    const val HISTORY_MAX_ITEMS = 10
    const val HISTORY_MAX_SIZE = 6 * 1024 * 1024
    const val RecycleBinName = "Recycle Bin"
    const val UntitledLabel = "Untitled"
}

enum class MemoryProtectionFlag(val value: String) {
    Title(XmlTags.MEMORY_PROTECTION_NAME),
    UserName(XmlTags.MEMORY_PROTECTION_USERNAME),
    Password(XmlTags.MEMORY_PROTECTION_PASSWORD),
    Url(XmlTags.MEMORY_PROTECTION_URL),
    Notes(XmlTags.MEMORY_PROTECTION_NOTES);
}
