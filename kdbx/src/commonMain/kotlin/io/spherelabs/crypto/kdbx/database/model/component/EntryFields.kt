...
class EntryFields(
    ...
    val title
        get() = fields[io.spherelabs.crypto.kdbx.database.BasicField.Title()]
    val userName
        get() = fields[io.spherelabs.crypto.kdbx.database.BasicField.UserName()]
    val password
        get() = fields[io.spherelabs.crypto.kdbx.database.BasicField.Password()]
    val url
        get() = fields[io.spherelabs.crypto.kdbx.database.BasicField.Url()]
    val notes
        get() = fields[io.spherelabs.crypto.kdbx.database.BasicField.Notes()]
     ...
     */
    operator fun minus(keys: Iterable<String>) = EntryFields(fields - keys.toSet())

    /**
     * Returns [EntryFields] containing all entries of the original except those entries
     * the keys of which are contained in the given [keys] array.
     */
    operator fun minus(keys: Array<String>) = EntryFields(fields - keys.toSet())

    /**
     * Returns [EntryFields] containing all entries of the original except those entries
     * the keys of which are contained in the given [keys] sequence.
     */
    operator fun minus(keys: Sequence<String>) = EntryFields(fields - keys.toSet())
    ...
    companion object {
        ...
        private fun createDefault() = EntryFields(
            buildMap {
                ...
                put(io.spherelabs.crypto.kdbx.database.BasicField.Password(), EntryValue.Encrypted(password))
            }
        )
    }
}
