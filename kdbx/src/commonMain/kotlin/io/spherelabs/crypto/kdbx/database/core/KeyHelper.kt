...
import io.spherelabs.crypto.kdbx.database.common.decodeHexToArray
...
class KeyHelper private constructor(
    val passphrase: InternalSecureBytes?,
    val key: InternalSecureBytes?,
) {
    companion object {
        fun from(passphrase: InternalSecureBytes): KeyHelper {
            return KeyHelper(
                passphrase = InternalSecureBytes.from(passphrase.sha256),
                key = null,
            )
        }
        ...
        fun from(key: ByteArray): KeyHelper {
            return KeyHelper(
                passphrase = null,
                key = InternalSecureBytes.from(parseKeyfile(key)),
            )
        }
