...
import io.spherelabs.crypto.kdbx.database.common.toArgon2EngineType
import io.spherelabs.crypto.kdbx.database.header.KdfParameters
...
internal fun transformKey(header: KdbxOuterHeader, keys: KdbxConfiguration): ByteArray {
    fun compositeKey(configuration: KdbxConfiguration): ByteArray {
        val hash = listOfNotNull(
            InternalSecureBytes.fromPlainText(configuration.passphrase ?: "").raw,
            InternalSecureBytes.fromPlainText(configuration.key ?: "").raw,
        )
