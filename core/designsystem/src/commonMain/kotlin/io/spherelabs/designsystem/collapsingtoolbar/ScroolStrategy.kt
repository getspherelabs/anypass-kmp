...
private class ScrollDelegate(private val offsetY: MutableState<Int>) {
...
internal class EnterAlwaysCollapsedNestedScrollConnection(
  ...
  override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
    ...
    val consumed =
        if (dy > 0) { // expanding: offset -> body -> toolbar
          ...
          val offsetConsumption =
              (dy - toolbarConsumption).coerceAtLeast(
                  -toolbarState.height.toFloat() - offsetY.value)
...
internal class ExitUntilCollapsedNestedScrollConnection(
  ...
  override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
    ...
    val consume =
        if (dy < 0) { // collapsing: toolbar -> body
  ...
  override fun onPostScroll(
    ...
    val consume =
        if (dy > 0) { // expanding: body -> toolbar
          toolbarState.dispatchRawDelta(dy)
        } else {
          0f
        }
